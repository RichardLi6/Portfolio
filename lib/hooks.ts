...
export function useSectionInView(sectionName: SectionName, threshold = 0.1) {
    const {ref, inView} = useInView({
      threshold: [0.25, 0.5], // this is the percentage of the section that needs to be in view for the inView to be, we need 3 types because it wouldn't show properly (buggy)
      });
